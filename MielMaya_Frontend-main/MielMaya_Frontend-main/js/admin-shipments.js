...
function renderOrders(orders) {
    ...
    orders.forEach(order => {
            ...
            <div class="shipment-main">
                <div class="shipment-product">
                    ...
                    <div class="product-details">
                        ...
                        <div class="order-items" data-order-items="${orderId}"></div>
                    </div>
                </div>
        ...
        fetch(`${API_BASE_URL}/orders/${orderId}/items`).then(r => r.ok ? r.json() : []).then(items => {
            const target = div.querySelector(`[data-order-items='${orderId}']`);
            if (!target) return;
            if (items && items.length) {
                target.innerHTML = `<div style="display:flex; gap:8px; flex-wrap:wrap;">
                  ${items.map(it => `
                    <div style="display:flex; align-items:center; gap:6px; border:1px solid #eee; border-radius:8px; padding:4px 8px;">
                      <img src='${it.imagen || DEFAULT_IMAGE}' alt='${it.nombre}' onerror="this.src='${DEFAULT_IMAGE}'" style='width:36px;height:36px;object-fit:cover;border-radius:6px;'>
                      <span>${it.nombre} × ${it.cantidad}</span>
                    </div>
                  `).join('')}
                </div>`;
            } else {
                target.innerHTML = `<p style='color:#888;'>Sin detalles</p>`;
            }
        }).catch(() => {
            const target = div.querySelector(`[data-order-items='${orderId}']`);
            if (target) target.innerHTML = `<p style='color:#888;'>Sin detalles</p>`;
        });
    });
