...
async function loadUserOrders() {
    ...
    try {
        ...
        const enriched = await Promise.all(orders.map(async (order) => {
            ...
            let items = [];
            try {
                const r = await fetch(`${API_BASE_URL}/orders/${orderId}/items`);
                if (r.ok) items = await r.json();
            } catch {}
            const itemsHtml = (items && items.length > 0)
                ? `<div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:8px;">
                     ${items.map(it => `
                       <div style="display:flex; align-items:center; gap:8px; border:1px solid #eee; border-radius:8px; padding:6px 10px;">
                         <img src="${it.imagen || DEFAULT_IMAGE}" alt="${it.nombre}" onerror="this.src='${DEFAULT_IMAGE}'" style="width:40px; height:40px; object-fit:cover; border-radius:6px;">
                         <span>${it.nombre} × ${it.cantidad}</span>
                       </div>`).join('')}
                   </div>`
                : `<p style="margin-top:8px; color:#888;">Sin detalles disponibles</p>`;
                        ...
                        <div class="order-dropdown">
                            <button class="order-action-btn repurchase-btn" onclick="repurchaseOrder(${orderId})">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0z"/>
                                </svg>
                                VOLVER A COMPRAR
                            </button>
                            ${canCancel ? `
        ...
        ordersList.innerHTML = enriched.join('');
